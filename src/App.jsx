...
const App = () => {
  ...
  const TransactionsView = ({ sellers, drivers, db, transactions }) => {
    ...
    const handleInputChange = (e) => {
      ...
      setNewItem({ ...updatedItem, netProfitCzk: netProfit.toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) });
    };
        ...
        <div className={cardClass}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">~ {(parseFloat(newItem.purchasePricePln || 0) * exchangeRate).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CZK</p>
            </div>
            <div>
        ...
        <div className={cardClass}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-indigo-50 rounded-xl p-4 shadow-sm">
              ...
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.purchasePricePln||0)*exchangeRate),0).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Prodej (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.sellingPriceCzk||0)),0).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
            <div className="bg-yellow-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Čistý zisk (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.netProfitCzk||0)),0).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
...
<div className="overflow-x-auto">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTransactions.sort((a, b) => b.saleDate < a.saleDate ? 1 : -1).map(item => (
                  <tr key={item.id}>
                    ...
                    <td className={tdClass}>{(parseFloat(item.purchasePricePln || 0) * exchangeRate).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td className={tdClass}>{Number((item.sellingPriceCzk || 0)).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td className={tdClass}>
                      <span className={`font-bold ${item.netProfitCzk > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {Number((item.netProfitCzk || 0)).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CZK
                      </span>
                    </td>
  ...
  const DeliveryFilterView = ({ transactions, drivers }) => {
    ...
    const handlePrint = () => {
  // Build printable HTML using current filteredDeliveries
  const rows = (filteredDeliveries || []).map((delivery, i) => {
    const itemName = delivery.itemName || "-";
    const brand = delivery.brand || "-";
    const model = delivery.model || "-";
    const customerName = delivery.customerName || "-";
    const customerAddress = delivery.customerAddress || "-";
    const customerContact = delivery.customerContact || "-";
    const destination = delivery.destination || "-";
    const note = delivery.note || "-";
    const saleDate = delivery.saleDate || "-";
    return `
      <tr>
        <td>${i + 1}</td>
        <td>${saleDate}</td>
  ...
  const printContents = `
    <html>
      <head>
        <meta charset="utf-8" />
        <title>Seznam rozvozů – ${selectedDriver || "Nezvolený řidič"} (týden ${weekNumber || "-"})</title>
        <style>
          body { font-family: -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans",sans-serif; margin: 20px; color: #111827; }
          h1 { font-size: 24px; margin: 0 0 4px 0; color: #111827; }
          h2 { font-size: 14px; margin: 0 0 16px 0; color: #6b7280; }
          table { width: 100%; border-collapse: collapse; margin-top: 12px; }
          th, td { border: 1px solid #e5e7eb; padding: 8px 10px; text-align: left; }
          th { background-color: #f9fafb; font-weight: 700; font-size: 12px; color: #374151; }
          td { font-size: 13px; }
          .meta { display:flex; justify-content: space-between; margin-bottom: 8px; font-size: 12px; color:#4b5563; }
          @media print {
      ...
      <body>
        <div class="meta">
          <div><strong>Řidič:</strong> ${selectedDriver || "-"}</div>
          <div><strong>Týden:</strong> ${weekNumber || "-"}</div>
          <div><strong>Vytištěno:</strong> ${new Date().toLocaleString("cs-CZ")}</div>
        </div>
  ...
  const SalesGraphView = ({ transactions, sellers }) => {
    ...
    useEffect(() => {
      ...
      const salesData = sellers.map(seller => {
        ...
        return {
          name: seller.name,
          'Čistý zisk': parseFloat(totalProfit.toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 })),
        };
      });
        ...
        <div className={cardClass}>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis dataKey="name" />
              <YAxis tickFormatter={(v) => Number(v || 0).toLocaleString("cs-CZ")} />
              <Tooltip formatter={(value) => Number(value || 0).toLocaleString("cs-CZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + " CZK"} />
              <Legend />
